...
class WorkerTaskManagerBak
{
    ...
    public function onTask(Server $server, $taskId, $reactorId, $json)
    {
        try {
            ...
            if ($res) {
                $this->logMessage('onTaskSuccess:' . $params['_id']);
            } else {
                $this->logMessage('onTaskFail:' . $params['_id']);
            }
        } catch (Exception $e) {
    ...
    {
        //多个task任务同时完成时，前一个会阻塞下一个onfinish
        try {
            $stats = $server->stats();
            ...
            //检查任务超时情况记录，删除，
            $this->deleteTimeoutKeys();
            $canDeliver = $this->taskAtomic->get() - $this->tableCount();
            if ($canDeliver > 0) {
                $taskModel = TaskFactory::factory($this->taskType);
                $taskModel->setType($this->type);
                for ($i = 0; $i < $canDeliver; $i++) {
